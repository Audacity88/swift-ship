...
export class QuoteAgent {
  ...
  constructor(baseUrl: string = 'https://dkrhdxqqkgutrnvsfhxi.supabase.co') {
    ...
    // Initialize OpenAI with tracing
  ...
  private collectQuoteDetails = traceable(async (messages: AgentMessage[]): Promise<{
  ...
  public process = traceable(async (context: AgentContext): Promise<AgentResponse> => {
    ...
    metadata: {
      ...
      this.log('Error in process:', error);
      const errorResponse = {
  ...
  private determineStep(messages: AgentMessage[]): Promise<'initial' | 'package_details' | 'addresses' | 'service_selection' | 'confirmation'> {
    if (messages.length === 0) return Promise.resolve('initial');
    ...
    if (lastAssistantMessage.includes('service option') || lastAssistantMessage.includes('Express Freight')) {
      return Promise.resolve('service_selection');
    }
    ...
    if (!packageDetails) {
      return Promise.resolve('package_details');
    }
